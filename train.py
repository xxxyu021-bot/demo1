...
from dataset import ToutiaoDataset, collate_fn, build_label_map
...
def main():
    ...
    label_map = build_label_map(cfg["data"]["train_txt_path"])
    ...
    # 构建数据集
    train_dataset = ToutiaoDataset(cfg["data"]["train_txt_path"], label_map)
    dev_dataset = ToutiaoDataset(cfg["data"]["dev_txt_path"], label_map)
    test_dataset = ToutiaoDataset(cfg["data"]["test_txt_path"], label_map)
    ...
    for epoch in range(cfg["num_epochs"]):
        ...
        val_loss, val_acc, val_recall, val_f1,val_precision = evaluate(model, dev_loader, criterion, device)
    ...
    test_loss, test_acc, test_recall, test_f1,test_precision = evaluate(model, test_loader, criterion, device)
    print(
